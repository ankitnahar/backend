...
class SendmailCommand extends Command {
    ...
    public function handle() {
        try {
            ...
            $blankemail = \App\Models\Backend\EmailContent::where("content", "=", "''")->where("content", "=", "null");
            ...
            /* $sqlUpdateTwo = \App\Models\Backend\EmailContent::where("status", "2")
              ->where("created_on", ">=", "$halfHourAgo")
              ->where("created_on", "<=", "$startTimeThree")->update(["status" => "0"]); */           
            $emailList = \App\Models\Backend\EmailContent::where('status', 0)->where("content", "!=", "''")->limit(100)->get();
            if (!empty($emailList) && count($emailList) > 0) {
                foreach ($emailList as $key => $value) {
                    ...
                    if ($value->to_email != '' && $value->subject != '') {
                        ...
                        /* $reTo = array();
                          $toArray = explode(",",$value->to_email);
                          for($i=0;$i<count($toArray);$i++){
                          $unemail = App\Models\Backend\NewsletterUnsubscribe::where("email",trim($toArray[$i]));
                          if($unemail->count() == 0){
                          $reTo[] = trim($toArray[$i]);
                          }
                          }
                          ...
                          $emailSend = \App\Models\Backend\EmailContent::find($value->id);
                          $emailSend->delete();
                          } */
            ...
            \App\Models\Backend\EmailServerIssue::create(["email_reason" => $ex->getMessage()]);
