...
class BillingSubactivityReportController extends Controller {
    ...
    public function generateReport(Request $request) {
        ...
        $alias = array("is_active" => "entity_bank_info", "billing_name" => "e", "service_id" => "billing_subactivity", "created_on" => "billing_subactivity");
        $fieldArray = array();
        ...
        foreach ($outputField as $field) {
            if (in_array($field, $designationField)) {
                ...
                if ($field == 'subactivity_code') {
                    $field = "billing_subactivity" . "." . "subactivity_code";
                }
        ...
        $generateReport = \App\Models\Backend\BillingServicesSubactivity::getSubactivityReportData()->select(DB::raw($selectField));
        ...
        if ($request->has('search')) {
            ...
            $alias = array("entity_name" => "e", "trading_name" => "e", "billing_name" => "e", "service_id" => "billing_subactivity", "created_on" => "billing_subactivity","parent_id" => "e","subactivity_code"=>"billing_subactivity");
        ...
        $generateReport = $generateReport->groupBy("e.id", "billing_subactivity.id");
       // showArray(getSQl($generateReport));exit;
        ...
        $generateReport = \App\Models\Backend\BillingServicesSubactivity::reportArrangeData($generateReport);
        ...
        if ($request->has('excel') && $request->get('excel') == 1) {
            ...
            return exportExcelsheet($column, 'BillingSubactivityReport', 'xlsx', 'A1:' . $x . '1');
        }
