...
class BillingReportController extends Controller {
    ...
    public function generateReport(Request $request) {
        ...
        $sortBy = ($request->has('sortBy')) ? $request->get('sortBy') : 'billing_basic.id';
        $sortOrder = ($request->has('sortOrder')) ? $request->get('sortOrder') : 'asc';
        $pager = [];

        //for field who need alias
        $alias = array("trading_name" => "e", "billing_name" => "e", "created_on" => "billing_basic","parent_id" => "ep");
        ...
        $fieldArray[] = "e.parent_id AS `parent_entity`";

        $selectField = implode(",", $fieldArray);
        //showArray($selectField);exit;
        $generateReport = \App\Models\Backend\Billing::getBillingReportData()->select(DB::raw($selectField));
        ...
        $generateReport = $generateReport->groupBy("e.id");
        ...
        $generateReport = \App\Models\Backend\Billing::reportArrangeData($generateReport);
        ...
        if ($request->has('excel') && $request->get('excel') == 1) {
            ...
            return exportExcelsheet($column, 'BillingReport', 'xlsx', 'A1:' . $x . '1');
        }
