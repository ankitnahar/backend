...
class RsheetReportController extends Controller {
    ...
    public function generateReport(Request $request) {
        ...
        $alias = array("is_active" => "worksheet", "task_id" => "worksheet", "question_id" => "wtc", "created_on" => "wl", "status_id" => "worksheet", "comment" => "wtc","parent_id" => "e");
        $fieldArray = array();
        $outputField = explode(",", $request->get('output_field'));
        $designationField = array(9);
        ...
        $generateReport = \App\Models\Backend\Worksheet::getReviewReport()->select(DB::raw($selectField));
        ...
        if ($request->has('search')) {
            ...
            $alias = array("entity_name" => "e", "trading_name" => "e", "billing_name" => "e", "comment" => "wtc",
                "is_active" => "worksheet", "task_id" => "worksheet", "question_id" => "wtc", "created_on" => "wl", "status_id" => "worksheet");
        ...
        //$generateReport = $generateReport->groupBy("wtc.question_id")->groupBy("wtc.worksheet_id");
        ...
        if ($request->has('records') && $request->get('records') == 'all') {
            ...
            $generateReport = $generateReport->skip($skip)
                    ->take($take);
        ...
        $generateReport = \App\Models\Backend\Worksheet::reportArrangeData($generateReport);
