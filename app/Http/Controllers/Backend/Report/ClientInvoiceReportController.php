...
class ClientInvoiceReportController extends Controller {
    ...
    public function generateReport(Request $request) {
        ...
        $sortBy = ($request->has('sortBy')) ? $request->get('sortBy') : 'invoice.entity_id';
        $sortOrder = ($request->has('sortOrder')) ? $request->get('sortOrder') : 'asc';
        $pager = [];

        //for field who need alias
        $alias = array("is_active" => "entity_bank_info", "service_id" => "invoice", "created_on" => "invoice","parent_id" => "e");
        $fieldArray = array();
        $outputField = explode(",", $request->get('output_field'));
        $designationField = array(9, 10, 15, 14);
        $sumArray = array('surcharge_amount', 'paid_amount', 'timesheet_unit', 'carry_unit', 'fixed_unit', 'extra_unit', 'woff_unit', 'won_unit', 'charged_unit',
            'total_charge_unit', 'extra_woff', 'extra_won');
        foreach ($outputField as $field) {
            ...
            } else if (in_array($field, $sumArray)) {
                $fieldTitle = \App\Models\Backend\ReportField::
                        where("field_name", $field)
                        ->where("tab_id", "35")
                        ->first();
                $fieldArray[] = "TRUNCATE(SUM(invoice." . $field . "),2) as `" . $fieldTitle['field_title'] . "`";
            } else {
        ...
        //showArray($selectField);exit;
        $generateReport = \App\Models\Backend\Invoice::getclientWiseInvoiceData()->select(DB::raw($selectField));
        ...
        $generateReport = $generateReport->groupby("invoice.entity_id");
